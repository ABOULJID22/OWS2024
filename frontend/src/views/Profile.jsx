...
const Profile = () => {
  ...
  useEffect(() => {
    axiosClient.get("/user").then(({ data }) => {
      setUser(data);
      // Récupérer la liste d'amis de l'utilisateu
  ...
  const onDeleteClick = async (friend) => {
    try {
      await axiosClient.delete(`/user/${user.id}/friends/${friend.id}`);
  ...
  const updateFriendsList = async () => {
    try {
      const { data: updatedFriendsData } = await axiosClient.get(`/user/${user.id}/friends`);
      setFriends(updatedFriendsData);
    } catch (error) {
      console.error("Erreur lors de la récupération de la liste d'amis après la suppression :", error);
    }
  };
...
// ...
  ...
  return (
    <div className="h-full bg-gray-200 p-8">
      ...
      <div className="bg-white rounded-lg shadow-xl pb-8">
        ...
        <Link to="/Home">
  <button className="w-full flex items-center px-6 py-1.5 space-x-2">
    <svg
      xmlns="http://www.w3.org/2000/svg"
      className="h-4 w-4 text-gray-400"
      fill="none"
    ...
    <span className="text-sm text-gray-700">Retour</span>
      ...
      <div className="flex flex-col md:flex-row md:space-x-4">
        <div className="w-full flex flex-col 2xl:w-1/3 p-4 ">
  ...
  <h4 className="text-xl text-gray-900 font-bold">Mes amis</h4>
  ...
  <ul className="mt-4 space-y-2">
    {friends.map((friend) => (
      <li key={friend.id} className="flex items-center space-x-4">
        ...
        <button className="btn-delete" onClick={(ev) => onDeleteClick(friend)}>
                        Delete
           </button>
